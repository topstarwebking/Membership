  ...
  return async (dispatch) => {
    try {
      ...
      if (res.data?.success) {
        ...
        toast.info(
          res.data?.msg + " please select valid smartlist",
          toastCSS()
        );
      }
    } catch (error) {
...
export const UPDATE_TEMPLATE_TO_EMAIL = (
  UrlPath,
  payload,
  template_id,
  F_id
) => {
  let url = `${domain}${UrlPath}/update_template/${getUserId()}/${template_id}`;
  return async (dispatch) => {
    try {
      ...
      if (response.data?.success) {
        toast.info(response?.data?.msg, toastCSS());
        dispatch(GET_SCHEDULE_MAILS(UrlPath, F_id));
      }
    } catch (error) {
      toast.error(error.message.replace(/\\/g, ""), toastCSS());
    }
...
export const MAKE_TEMPLATE_AS_FAVORITES = (
  UrlPath,
  payload,
  template_id,
  F_id
) => {
  let url = `${domain}${UrlPath}/marks_as_star/${getUserId()}/${template_id}`;
  return async (dispatch) => {
    try {
      let response = await axios.put(url, payload, {
        headers: getHeaders(),
...
export const MAKE_TEMPLATE_AS_ACTIVATE = (
  UrlPath,
  payload,
  F_id
) => {
