...
import CreatAndEditdolder from "./CreateAndEditfolder";
...
import EditorDeletSmratlist from "./EditorDeletSmratlist";
...
const SmartListsidebar = (props) => {
  const classes = useStyles();
  const [anchorEl, setAnchorEl] = React.useState(null);
  const open = Boolean(anchorEl);
  const { getAllSmartList, DELETE_SMART_LIST, userinformation } = props;
          ...
          {getAllSmartList?.map((item, i) => {
            return (
              <ListItem
                key={item?._id}
                className={`${
                  activeFolderdata?._id === item?._id && classes.activeFolder
                }`}
              >
                <Button
                  fullWidth
        ...
        <div className="w-100">
          ...
          <div className="w-100 m-1">
            <Grid container spacing={1} className="w-100">
              {activeFolderdata?.smartlists?.length > 0 &&
                activeFolderdata?.smartlists?.map((element) => {
                  return (
                    <Grid item sm={12} md={3} lg={3} key={element?._id}>
                      <div
                        ...
                        <div className="p-1">
                          ...
                          >
                            <div className="d-flex">
                              <Avatar
                                style={{
                                  background: hexToRGB(getRandomBG(), 0.2),
                                }}
                              >
